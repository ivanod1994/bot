...
DEFAULT_TIMEFRAME = "1m"  # Изменено с 5m на 1m
CSV_FILE = "signals.csv"
DELETE_AFTER_MINUTES = 10  # Увеличено с 5
PREPARE_SECONDS = 90
RESULT_LOG_FILE = "/app/data/results_log.csv"
MANUAL_TZ = "Africa/Algiers"
CONFIRMATION_CANDLES = 2
TIMEOUT = 90
MIN_SIGNAL_INTERVAL = 300  # Увеличено с 60 (5 минут)
...
def get_correlation_confirmation(symbol, signal, df_5m, expiration=5):  # expiration=5
...
def get_data(symbol, interval=DEFAULT_TIMEFRAME, period="1d"):
    ...
    for attempt in range(3):
        try:
            # Для минутных интервалов загружаем данные за 1 день
            url = f"https://query1.finance.yahoo.com/v8/finance/chart/{symbol}?range={'1d' if interval in ['1m', '2m', '5m'] else period}&interval={interval}"
            response = session.get(url, headers=HEADERS, timeout=TIMEOUT)
...
def update_outcome(symbol, entry_time, entry_price, signal, expiration, stop_loss, take_profit):
    ...
    try:
        ...
        end_time = entry_dt + timedelta(minutes=5)  # Экспирация 5 минут
        ...
        # Расчет уровней для минимального профита/убытка
        price_high = entry_price * (1.00005 + 0.00003 * expiration)
        price_low = entry_price * (0.99995 - 0.00003 * expiration)
...
def analyze(symbol, df_5m, df_15m=None, df_1h=None, expiration=5):  # expiration=5
    ...
    try:
        ...
        rsi_window = 5 if expiration == 5 else 7 if expiration == 2 else 10
        ...
        if trend_h1 == "BEARISH" and h1_trend_confirmed:
            signal_stрегь += weights['trend'] * 0.5
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    df_5m = get_data(symbol, interval="1m", period="1d")  # 1m данные
    ...
    signal, rsi, signal_strength, price, atr, reason, rsi_v, adx_v, stoch_v, macd_v, signal_v, success_prob, stop_loss, take_profit, lot_size = analyze(symbol, df_5m, df_15m, df_1h, expiration=5)  # expiration=5
    ...
    if signal in ["BUY", "SELL"]:
        ...
        msg = (f"📊 *Сигнал для {symbol}*\n"
               ...
               f"⏰ Время: {entry_time} | Экспирация: 5 минут\n"
...
async def check_results(context: ContextTypes.DEFAULT_TYPE):
    ...
    for index, row in df.iterrows():
        ...
        if (current_time - entry_time).total_seconds() / 60 > DELETE_AFTER_MINUTES:
            outcome, exit_price = update_outcome(row['Symbol'], row['Entry_Time'], row['Entry_Price'], row['Signal'], 5, row['Stop_Loss'], row['Take_Profit'])  # expiration=5
...
async def auto_analyze(context: ContextTypes.DEFAULT_TYPE):
    ...
    for symbol in SYMBOLS:
        ...
        print(f"[{datetime.now(LOCAL_TZ).strftime('%H:%M:%S')}] Анализ {symbol} на таймфрейме {DEFAULT_TIMEFRAME} с экспирацией 5 мин")
        df_5m = get_data(symbol, interval="1m", period="1d")  # 1m данные
        df_15m = get_data(symbol, interval="15m", period="7d")
        df_1h = get_data(symbol, interval="1h", period="30d")
        if df_5m is None:
...
def main():
    ...
    application.job_queue.run_repeating(check_results, interval=30, first=10)
