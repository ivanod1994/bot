...
def analyze(symbol, df_5m, df_15m=None, df_1h=None, expiration=1):
    ...
    # Риск-менеджмент
    stop_loss = price * (0.999 - atr_v * 1.5) if signal == "BUY" else price * (1.001 + atr_v * 1.5)
    take_profit = price * (1.001 + atr_v * 2.0) if signal == "BUY" else price * (0.999 - atr_v * 2.0)
    ...
    reason = (f"RSI: {rsi_v:.2f}, ADX: {adx_v:.2f}, Stochastic: {stoch_v:.2f}, MACD: {macd_val:.4f}, "
              ...
              f"Stop Loss: {stop_loss:.4f}, Take Profit: {take_profit:.4f}, Lot Size: {lot_size:.2f}")

    # Проверка активных сделок
    df_results = pd.read_csv(RESULT_LOG_FILE) if os.path.exists(RESULT_LOG_FILE) else pd.DataFrame()
    active_trades = len(df_results[df_results['Outcome'] == 'PENDING'])
    if active_trades >= MAX_ACTIVE_TRADES:
        reason += f"; Превышен лимит активных сделок ({MAX_ACTIVE_TRADES})"
        print(f"[{datetime.now(LOCAL_TZ).strftime('%H:%M:%S')}] {symbol}: {reason}")
        return "WAIT", round(rsi_v, 2), 0, price, atr_v, reason, rsi_v, adx_v, stoch_v, macd_val, signal_val, success_probability, stop_loss, take_profit, lot_size
    ...
    if reward_risk_ratio < 1.5:
        reason += f"; Низкое соотношение риск/прибыль ({reward_risk_ratio:.2f} < 1.5)"
        print(f"[{datetime.now(LOCAL_TZ).strftime('%H:%M:%S')}] {symbol}: {reason}")
        return "WAIT", round(rsi_v, 2), 0, price, atr_v, reason, rsi_v, adx_v, stoch_v, macd_val, signal_val, success_probability, stop_loss, take_profit, lot_size
    ...
    reason += f"; Недостаточно условий для сигнала или низкая вероятность ({success_probability:.2%} < {MIN_SUCCESS_PROBABILITY:.2%})"
    print(f"[{datetime.now(LOCAL_TZ).strftime('%H:%M:%S')}] {symbol}: {reason}")
    return "WAIT", round(rsi_v, 2), 0, price, atr_v, reason, rsi_v, adx_v, stoch_v, macd_val, signal_val, success_probability, stop_loss, take_profit, lot_size
    ...
    for attempt in range(3):
        try:
            await update.message.reply_text(
                "Добро пожаловать в торгового бота! Прогнозы оптимизированы для прибыльности с риск-менеджментом. Используйте кнопки для выбора пары, экспирации или силы сигнала.",
...
async def run_analysis(context: ContextTypes.DEFAULT_TYPE):
    ...
    min_signal_strength = context.bot_data.get('auto_signal_strength', MIN_SIGNAL_STRENGTH)
    ...
    for symbol in SYMBOLS:
        ...
        try:
            ...
            signal, rsi, strength, price, atr_v, reason, rsi_v, adx_v, stoch_v, macd_val, signal_val, success_probability, stop_loss, take_profit, lot_size = analyze(symbol, df, df_15m, df_1h, expiration)
            ...
            if signal != "WAIT":
                ...
                if strength >= min_signal_strength:
                    ...
                    outcome, exit_price = update_outcome(symbol, entry_time, price, signal, expiration, stop_loss, take_profit)
                    msg = (
                    ...
                    log_result(symbol.replace('=X',''), signal, rsi, entry_time, reason, rsi_v, adx_v, stoch_v,
                               macd_val, signal_val, atr_v, price, exit_price, success_probability, outcome, stop_loss, take_profit, lot_size)
                    ...
                    if len(df_results[df_results['Outcome'].isin(['WIN', 'LOSS'])]) % 10 == 0:
                        ...
                        win_profit = (df_results[df_results['Outcome'] == 'WIN']['Exit_Price'] - df_results[df_results['Outcome'] == 'WIN']['Entry_Price']).mean()
                        loss_loss = (df_results[df_results['Outcome'] == 'LOSS']['Entry_Price'] - df_results[df_results['Outcome'] == 'LOSS']['Exit_Price']).mean()
                        profit_loss_ratio = win_profit / loss_loss if loss_loss != 0 else float('inf')
                        msg = (f"📊 Промежуточный отчёт:\n"
                               f"Win Rate: {win_rate:.2%}\n"
                               f"Сигналов: {len(df_results)}\n"
                               f"Profit/Loss Ratio: {profit_loss_ratio:.2f}\n"
...
def log_result(symbol, signal, rsi, entry_time, reason, rsi_v, adx_v, stoch_v, macd_val, signal_val, atr_v, entry_price, exit_price, success_probability, outcome, stop_loss, take_profit, lot_size):
    expected_columns = ["Symbol", "Signal", "RSI", "Entry Time", "Logged At", "Reason", "Outcome",
                       "RSI_Value", "ADX_Value", "Stochastic_Value", "MACD_Value", "Signal_Value",
                       "ATR_Value", "Entry_Price", "Exit_Price", "Stop_Loss", "Take_Profit",
                       "Lot_Size", "Success_Probability"]
    try:
        ...
        with open(RESULT_LOG_FILE, 'a', newline='', encoding='utf-8') as f:
            csv.writer(f).writerow([symbol, signal, rsi, entry_time, datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S"),
                                    reason, outcome, rsi_v, adx_v, stoch_v, macd_val, signal_val, atr_v,
                                    entry_price, exit_price, stop_loss, take_profit, lot_size, success_probability])
    ...
    try:
        ...
        if not os.path.exists(RESULT_LOG_FILE):
            ...
            with open(RESULT_LOG_FILE, 'w', newline='', encoding='utf-8') as f:
                csv.writer(f).writerow(["Symbol", "Signal", "RSI", "Entry Time", "Logged At", "Reason", "Outcome",
                                        "RSI_Value", "ADX_Value", "Stochastic_Value", "MACD_Value", "Signal_Value",
                                        "ATR_Value", "Entry_Price", "Exit_Price", "Stop_Loss", "Take_Profit",
                                        "Lot_Size", "Success_Probability"])
